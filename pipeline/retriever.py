...
def semantic_search(
    ...
    if domain_filters:
        ...
        where_params.append(domain_filters)

    if doc_type_filters:
        conditions.append("ld.doc_type = ANY(%s)")
        where_params.append(doc_type_filters)
...
def retrieve(
    ...
    Args:
        ...
        domain_filters:  Optional — list of legal domains to filter
        doc_type_filters:Optional — list of doc types to filter
    ...
    semantic_results = semantic_search(
        conn, query_embedding, candidates, domain_filters, doc_type_filters
    )
    bm25_results = bm25_search(
        conn, question, candidates, domain_filters, doc_type_filters
    )
